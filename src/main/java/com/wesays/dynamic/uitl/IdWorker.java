package com.wesays.dynamic.uitl;
...
public class IdWorker {
    ...
    // 机器ID最大值
    private static final long MAXWORKERID = DEFAULT ^ (DEFAULT << WORKERIDBITS);
    // 数据中心ID最大值
    private static final long MAXDATACENTERID = DEFAULT ^ (DEFAULT << DATACENTERIDBITS);
    // 毫秒内自增位
    ...
    private static final long SEQUENCEMASK = DEFAULT ^ (DEFAULT << SEQUENCEBITS);
    /* 上次生产id时间戳 */
    private static long lastTimestamp = DEFAULT;
    // 0，并发控制
    ...
    protected static long getMaxWorkerId(long datacenterId, long maxWorkId) {
         ...
         */
        return (mpid.toString().hashCode() & 0xffff) % (maxWorkId + 1);
    }
